...
The length of the scene (eg. the highest numbered keyframe in its definition) will control the length of the created scroll pane. For every whole number a new page will be added.
<Alert type="info">
In the example above, the last keyframe is at *progress* 1. When using the 
</Alert>
