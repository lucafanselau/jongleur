...
export const sceneAnimation = orchestrate(
  ...
  },
  {
    ...
    second: {
      ...
      1: { opacity: [1, "ease-in-out"] },
      1.25: { opacity: [0, "ease-in-out"] }
    },
    third: {
      1.75: { opacity: InheritSymbol },
      2: { opacity: [1, "ease-in-out"] },
      2.25: { opacity: [0, "ease-in-out"] }
    },
    fourth: {
      2.75: { opacity: InheritSymbol },
      3: { opacity: [1, "ease-in-out"] },
      3.25: { opacity: [0, "ease-in-out"] }
    },
    fifth: {
      3.75: { opacity: InheritSymbol },
      4: { opacity: [1, "ease-in-out"] }
    },
    wall: {
      1.1: { scale: InheritSymbol },
      1.5: { scale: [new Vector3(1, 1, 1), "linear"] }
    },
    table: {
      1.1: { position: InheritSymbol },
      1.6: { position: [new Vector3(0, 0, 0), "ease-out"] }
    },
    tableStuff: {
      1.6: { scale: InheritSymbol },
      1.9: { scale: [new Vector3(1, 1, 1), "ease-out"] }
    },
    chair: {
      2: { scale: InheritSymbol },
      2.2: { scale: [new Vector3(1, 1, 1), "linear"], rotation: InheritSymbol },
      2.4: { position: InheritSymbol },
      2.6: { rotation: [new Vector3(0, -Math.PI / 2, 0), "ease-in-out"] },
      2.9: { position: [new Vector3(-50, 0, 0), "ease-out"] }
    },
    bed: {
      3: { scale: InheritSymbol },
      3.5: { scale: [new Vector3(1, 1, 1), "ease-out"] }
    },
    lamp: {
      3: { visible: InheritSymbol, position: InheritSymbol },
      3.8: { visible: [true, "start"], position: [new Vector3(0, 0, 0), "ease-out"] }
    },
    lampLights: {
      3.5: { intensity: InheritSymbol },
      4: { intensity: [1.5, "linear"] }
    }
  },
  5
);
